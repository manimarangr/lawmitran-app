...
export class AdminCreateUserDto {
  ...
  @IsIn(['CLIENT', 'LAWYER', 'ADMIN'], { message: 'Role must be CLIENT, LAWYER, or ADMIN' })
  role: 'CLIENT' | 'LAWYER' | 'ADMIN';

  @ApiPropertyOptional({ enum: ['SUPER', 'OPS', 'FINANCE'], description: 'Staff role — only when role=ADMIN (SUPER admins only)' })
  @IsOptional()
  @IsIn(['SUPER', 'OPS', 'FINANCE'])
  adminRole?: 'SUPER' | 'OPS' | 'FINANCE';

  @ApiPropertyOptional({ description: 'Omit to auto-generate a temporary password' })
  @IsOptional()
  @IsString()
  @MinLength(8)
  password?: string;
